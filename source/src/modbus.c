...
/** 串口转网络 **/
	...
	/* modbus rtu tcp 互转变量定义 */
	int mdbs_RT_ret;//RTU to TCP return
	int mdbs_TR_ret;//TCP to RTU return
	unsigned char* mdbs_RT_buf;//RTU to TCP buffer
	unsigned char* mdbs_TR_buf;//TCP to RTU buffer
	...
	/* modbustcp事务标识符共享内存相关变量 */
	unsigned short* mdbs_tranID = NULL;
	int shmid;
	char *shmadd;
	...
	/* 共享内存 */
	if((shmid=shmget(MDBS_SHM_ID,BUFSIZE,0666|IPC_CREAT))==-1){
		perror("shmget error");
		exit(1);
        }
        ...
        mdbs_tranID=(unsigned short*)shmadd;
...
restart:
	...
	/* 0号子进程MODBUS tcp到rtu */
		...
		mdbs_TR_buf = (unsigned char*) malloc(BUFFERSIZE);
		memset(net2serial_buf,0,BUFFERSIZE);
		memset(mdbs_TR_buf,0,BUFFERSIZE);
		...
		/* tcp 2 rtu */
				...
				//tcp 2 rtu
				mdbs_TR_ret=Modbus_TCP_RTU(net2serial_buf,mdbs_TR_buf,mdbs_tranID);
				//
                                writebytes = write(ser_fd,mdbs_TR_buf,mdbs_TR_ret);
                                if (writebytes < 0){
                                        perror("write");
	...
	/* 1号子进程MODBUS rtu到tcp */
		...
		mdbs_RT_buf = (unsigned char*) malloc(BUFFERSIZE);
		memset(mdbs_RT_buf,0,BUFFERSIZE);
		...
		/* rtu 2 tcp */
				...
				//rtu 2 tcp
				mdbs_RT_ret=Modbus_RTU_TCP(mdbs_RT_buf,serial2net_buf,readbytes,mdbs_tranID);
				//
                                sendbytes = send(sockfd,mdbs_RT_buf,mdbs_RT_ret,0);
                                if (sendbytes < 0){
                                        perror("send");
                                        printf("sendbytes=%d\r\n",sendbytes);//debug
					close(ser_fd);
	...
	/* 父进程 */
                ...
                waitpid(pid_arr[0],NULL,0);//服务器正常断开会触发网络接收子进程的关闭，父进程在这等待关闭
